...
#
# Responsibilities:
#   1. Map hostname to canonical VMnet10 IP per nexus-platform-plan/
#      docs/infra/vms.yaml (vault-1=.121, vault-2=.122, vault-3=.123)
#   2. Identify the secondary NIC (the one that's NOT nic0), rename it to
#      nic1 via systemd .link drop-in, assign the static VMnet10 IP
#   3. Generate fresh self-signed TLS cert per-clone with the clone's
#      actual hostname + IPs in SAN (Vault PKI in Phase 0.D.2 reissues)
#   4. Render /etc/vault.d/vault.hcl from vault.hcl.tpl, substituting
#      @HOSTNAME@, @VMNET11_IP@, @VMNET10_IP@
#   5. Mark complete (idempotent re-run guard)
#
...
# ─── 1. Map hostname to canonical VMnet10 IP ──────────────────────────────
# Authoritative source: nexus-platform-plan/docs/infra/vms.yaml lines 55-57.
case "$HOSTNAME" in
  vault-1) VMNET10_IP=192.168.10.121 ;;
  vault-2) VMNET10_IP=192.168.10.122 ;;
  vault-3) VMNET10_IP=192.168.10.123 ;;
  *)
    echo "$LOG_PREFIX ERROR: unknown hostname '$HOSTNAME' (expected vault-1/2/3 per vms.yaml)" >&2
    exit 1
    ;;
esac
echo "$LOG_PREFIX VMnet10 (backplane) IP: $VMNET10_IP/24"

# ─── 2. Identify + rename + configure the secondary NIC ───────────────────
...
# ─── 3. Get the VMnet11 IP (DHCP-assigned via dnsmasq dhcp-host reservation) ───
# nic0 should have its DHCP'd IP by now. Wait briefly if not.
VMNET11_IP=""
...
if [ -z "$VMNET11_IP" ]; then
  echo "$LOG_PREFIX ERROR: nic0 has no IPv4 address (DHCP failed?)" >&2
  exit 1
fi
echo "$LOG_PREFIX VMnet11 (service) IP: $VMNET11_IP"

# ─── 4. Generate self-signed TLS cert per-clone ───────────────────────────
...
# ─── 5. Render vault.hcl from template ────────────────────────────────────
