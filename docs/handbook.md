...
- **Stale `vmrun.exe` path** (x86 → non-x64 after the VMware upgrade) in clone_vm state → plan-check first ([[feedback_stale_vmrun_path_in_clone_vm_state]]).
  - **The `modules/vm` default fix (x86 → non-x64, 2026-06-18) makes the foundation env's EXISTING `clone_vm`/`power_on` state (dc-nexus / dc-nexus-2 / jumpbox) plan as "must be replaced"** — a `foundation.ps1 apply` would run the destroy-provisioner and delete the LIVE DCs' `.vmx`. Don't run a foundation apply until the state is reconciled. **Resolution (state-only; the running DCs are untouched) — done 2026-06-18:** back up `terraform/envs/foundation/terraform.tfstate`, literal-replace the 6 `"vmrun": "C:/Program Files (x86)/.../vmrun.exe"` trigger occurrences with the non-x64 path, then `terraform -chdir=terraform/envs/foundation plan` → **must read `No changes`**. After this the foundation env uses the non-x64 path consistently (default + state) and a `foundation.ps1 apply` is safe again. (A genuine DC rebuild destroys the env first, so the clone re-bakes the correct path regardless.)
