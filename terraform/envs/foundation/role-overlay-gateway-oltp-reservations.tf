 ...
 * (v3); 0.G.4 extends with 8 Patroni-tier reservations (v5 -- this version;
 * v4 was the single-HAProxy variant superseded mid-scaffold by the HA pair
 * design to align with the proxysql-1/2 pattern). Later 0.G sub-phases extend
 * further: SQL FCI/AG 0.G.7 -> .11-.14. The single-file shape avoids
 ...
 *
 * The ProxySQL VIP .50 + the HAProxy VIP .60 are NOT dhcp reservations --
 * they float between their respective LB pairs via keepalived/VRRP unicast,
 * configured per-node by the oltp env (which owns the per-node LB +
 * keepalived config). VMware Workstation's VMnet11 doesn't reliably forward
 * IPv4 multicast 224.0.0.18 -- lesson baked at 0.G.3.5c chunk 1 transient
 * #22; both VRRP pairs use unicast for that reason.
 *
...
resource "null_resource" "gateway_oltp_reservations" {
  ...
  triggers = {
    ...
    oltp_reservations_v = "5" # v5 (0.G.4) = +8 Patroni-tier reservations (pg-primary/pg-replica-{1,2} .61-.63, etcd-{1,2,3} .64-.66, haproxy-pg-{1,2} .67-.68). v4 was the single-HAProxy variant, superseded mid-scaffold by the HA pair design. v3 (0.G.3) added +5 PXC/ProxySQL (.51-.55). v2 (0.G.2) added +3 mongo (.71-.73). v1 was redis only.
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $marker  = '# OLTP tier dhcp-host reservations managed by terraform/envs/foundation/role-overlay-gateway-oltp-reservations.tf v5'

      # Idempotent insert: marker matches v5 specifically. Earlier versions
      # (v1 redis only, v2 redis+mongo, v3 +percona, v4 the abandoned single-
      # HAProxy variant) all get replaced atomically via write-through tee.
      ...
      if ($existing -match [regex]::Escape($marker)) {
        Write-Host "[gateway oltp-reservations] v5 reservations already present, no-op."
        exit 0
      }

      # Per nexus-platform-plan/docs/infra/vms.yaml (clusters: redis + mongo + percona + postgres).
      $confLines = @(
        $marker
...
"@
      Write-Host "[gateway oltp-reservations] writing 22 dhcp-host reservations (6 Redis + 3 Mongo + 3 PXC + 2 ProxySQL + 3 Patroni + 3 etcd + 2 HAProxy) + restarting dnsmasq..."
      ssh nexusadmin@$gw $script
      if ($LASTEXITCODE -ne 0) { throw "[gateway oltp-reservations] ssh tee/restart failed (rc=$LASTEXITCODE)" }
      Write-Host "[gateway oltp-reservations] reservations live: redis-1..6 (.81-.84/.87/.89), mongo-1..3 (.71/.72/.73), pxc-node-1..3 (.51/.52/.53), proxysql-1..2 (.54/.55), pg-primary/pg-replica-1/pg-replica-2 (.61-.63), etcd-1..3 (.64/.65/.66), haproxy-pg-1..2 (.67/.68)"
    PWSH
  }
